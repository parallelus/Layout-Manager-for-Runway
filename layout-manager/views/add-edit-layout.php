	...
	<div id="post-body" class="metabox-holder columns-2">
		<div id="post-body-content">
			...
			<div>
				<div id="titlediv">
					<div id="titlewrap">
						<label class="hide-if-no-js" style="visibility: hidden; " id="title-prompt-text" for="title">
							<?php _e( 'Enter title here', 'runway' ); ?>
						</label>
						<input type="text" name="post_title" size="30" tabindex="1"
						       value="<?php esc_attr_e( $layout['title'] ); ?>" id="title" autocomplete="off">
					</div>
				</div>

				<h3 id="big-header" class="layout-heading"><?php _e( 'HEADER', 'runway' ); ?></h3>
				<div id="HeaderWrapper">
					<div class="control-bar">
						<div id="HeaderAddNew">
							<a href="#add_header" id="AddHeader" class="button"><?php _e( 'Add New', 'runway' ); ?></a>
						</div>
						<div id="HeaderSelect">
							...
							<a href="#edit_header" id="EditHeader" class="button"><?php _e( 'Edit', 'runway' ); ?></a>
						</div>
						<div id="HeaderAddEdit">
							<input type="text" name="header_title" class="header-title">
							<a href="#save_header" id="SaveHeader"
							   class="button button-primary"><?php _e( 'Save', 'runway' ); ?></a>
							<a href="#cancel_edit_header" id="CancelEditHeader"
							   class="button"><?php _e( 'Cancel', 'runway' ); ?></a>
						</div>
					</div>
				...
				<h3 class="layout-heading"><?php _e( 'BODY', 'runway' ) ?></h3>
				<div id="GridWrapper">
					<div id="GridEnd" class="not-a-row">
						<a href="#add_row_button" id="AddNewRow"
						   class="button button-hero"><?php _e( 'Add Row', 'runway' ); ?></a>
					</div>
				</div><!-- / #GridWrapper -->

				<h3 id="big-footer" class="layout-heading"><?php _e( 'FOOTER', 'runway' ); ?></h3>
				<div id="FooterWrapper">
					<div class="control-bar">
						<div id="FooterAddNew">
							<a href="#add_footer" id="AddFooter" class="button"><?php _e( 'Add New', 'runway' ); ?></a>
						</div>
						<div id="FooterSelect">
							<span class="spinner"></span>
							...
							<a href="#edit_footer" id="EditFooter" class="button"><?php _e( 'Edit', 'runway' ); ?></a>
						</div>
						<div id="FooterAddEdit">
							<input type="text" name="footer_title" class="footer-title">
							<a href="#save_footer" id="SaveFooter"
							   class="button button-primary"><?php _e( 'Save', 'runway' ); ?></a>
							<a href="#cancel_edit_footer" id="CancelEditFooter"
							   class="button"><?php _e( 'Cancel', 'runway' ); ?></a>
						</div>
					</div>
				...
				<div id="ColumnTemplate">
					...
					<div class="column">
						<div class="column-header">
							<a href="#desktop" id="desktop" class="responsive-visibility show-on desktop-view"
							   title="<?php _e( 'Visible on Desktop', 'runway' ); ?>"></a>
							<a href="#tablet" id="tablet" class="responsive-visibility show-on tablet-view"
						...
						<div class="content-element">
							<div class="handle"></div>
								...
								<select name="content_element_type" class="content-element-type">
									<option value="default_content"><?php _e( 'Default Content', 'runway' ); ?></option>
									...
									foreach ( $custom_post_types as $key => $custom_post_type ) {
										...
										if ( count( $custom_posts[ $key ] ) > 0 ) {
											if (
												isset( $layouts_manager->layouts_manager_options['settings']["content-source-$key"] )
									...
									if (
										isset( $layouts_manager->layouts_manager_options['settings']['content-source-posts'] )
									    && $layouts_manager->layouts_manager_options['settings']['content-source-posts'] == 'true'
									) { ?>
										<option value="post"><?php _e( 'Post', 'runway' ); ?></option>
									<?php }

									if (
										isset( $layouts_manager->layouts_manager_options['settings']['content-source-pages'] )
										&& $layouts_manager->layouts_manager_options['settings']['content-source-pages'] == 'true'
									) { ?>
										<option value="page"><?php _e('Page', 'runway'); ?></option>
									<?php }

									if (
										isset( $layouts_manager->layouts_manager_options['settings']['content-source-sidebars'] )
										&& $layouts_manager->layouts_manager_options['settings']['content-source-sidebars'] == 'true'
									) { ?>
										<option value="sidebar"><?php _e('Sidebar', 'runway'); ?></option>
									<?php } ?>

								</select>
								<p class="content-type-help-text">
									<span class="spinner"></span>
									<span class="description-default_content">
										<?php _e( 'The default content of the area. <strong>This should exist only once in every layout.</strong>', 'runway' ); ?>
									</span>
									...
									foreach ( $custom_post_types as $key => $custom_post_type ) {
										...
										if ( count( $custom_posts[ $key ] ) > 0 ) {
											if (
												...
												<span class="description-<?php echo $key; ?>">
													<?php _e( "Choose a $custom_post_type_label from as the content source.", 'runway' ); ?>
												</span>
											<?php }
										}
									}
									...
									<span class="description-page"><?php _e( 'Choose a page from as the content source.', 'runway' ); ?></span>
									<span class="description-sidebar"><?php _e( 'Select a sidebar to use as the content source.', 'runway' ); ?></span>
								</p>
								<div class="secondary-select" style="display: none;">
									...
									<div class="check-override-secondary">
										<label>
											<input class="input-check-secondary" type="checkbox" value="false" name="allow_override">
											<?php _e( 'Allow override on single pages', 'runway' ); ?>
										</label>
										<p class="optional-label-override-secondary">
											...
											<p class="optional-label-help-text-secondary">
												<?php _e( 'Custom label for override option.', 'runway' ); ?>
											</p>
										</p>
									</div>
								</div>
						...
						foreach ( $custom_post_types as $key => $custom_post_type ) {
							...
							if ( count( $custom_posts[ $key ] ) > 0 ) {
								if (
									...
									<span class="description-<?php echo $key; ?>">
										<?php echo sprintf( __( "Choose a %s from as the content source.", 'runway' ), $custom_post_type_label ); ?>
									</span>
								<?php }
							}
						} ?>
		...
		<div id="postbox-container-1" class="postbox-container">
			...
			<div class="meta-box-sortables metabox-holder">
				<div class="postbox">
					...
					<h3 class="hndle"><span><?php _e( 'Layout Options', 'runway' ); ?></span></h3>
					<div class="inside">
						...
						if ( isset( $layouts_manager->layouts_manager_options['settings']['other-options'] ) ) {
							...
							if ( $form_json != '' ) {
								...
								$form_builder->render_form(
									...
									false,
									$layouts_manager,
									$layout_manager_admin,
									'other_options_' . $layout['alias']
								); // render form in one function call
							}
						...
						<p>
							<button style="float:right;" id="save_structure" class="button-primary ajax-save">
								<?php _e( 'Save settings', 'runway' ); ?>
							</button>
							<span class="spinner"></span>
							<div class="clear"></div>
						</p>
