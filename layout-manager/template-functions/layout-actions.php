...
// Generic function to look up header, footer or layout by alias
//................................................................
	...
	if ( ! isset( $layout ) || empty( $layout ) ) {
		...
		if ( is_array( $layout['body_structure'] ) ) {
			...
			if ( IS_CHILD && isset( $layouts_manager->layouts_manager_options['settings'] ) ) {
				$grid_type = ( false === strstr( $layouts_manager->layouts_manager_options['settings']['column-class-format'], '%' ) ) ?
					'fixed' :
					'fluid'; // for unsemantic
				if ( $grid_type == 'fluid' ) {
			...
			$row_class   = ( IS_CHILD && isset( $layouts_manager->layouts_manager_options['settings'] ) ) ?
				$layouts_manager->layouts_manager_options['settings']['row-class'] :
				'row-fluid';

			if ( $output ) {
				echo '<div id="' . $layout_name . '" class="grid-wrapper">' .
			...
			foreach ( (array) $rows as $columns ) {
				...
				if ( $output ) {
					echo '<section class="grid-section-' . $row_count . '">' .
					     '<div class="grid-row ' . $row_class . '">' . PHP_EOL;
				} // with line break
				...
				foreach ( (array) $columns as $column ) {
					...
					$gridPosition = ( isset( $column['grid_position'] ) ) ?
						$column['grid_position'] :
						$layouts_manager->layouts_manager_options['settings']['columns'];
					...
					if ( IS_CHILD && isset( $layouts_manager->layouts_manager_options['settings'] ) ) {
						$column_class_format = ( $grid_type == 'fixed' ) ?
...
function context_fallbacks_func( $layout ) {
	...
	$context = ( is_category() && empty( $layouts_manager->layouts_manager_options['contexts'][ $context ] ) ) ?
		'category' :
		$context;
